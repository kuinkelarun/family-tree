...
export default function RelationshipPicker({
  ...
  // Do not auto-populate the label based on the type. The label is truly optional
  // and should remain empty unless the user explicitly types a custom value.
  useEffect(() => {
    setType(defaultType);
    ...
    <div onClick={() => onCancel?.()} style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#ffffff', color: '#111827', borderRadius: 10, padding: 16, width: 340, boxShadow: '0 8px 24px rgba(0,0,0,0.25)', border: '1px solid #e5e7eb' }} role="dialog" aria-modal="true">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <label htmlFor="rel-type" style={{ fontSize: 12, color: '#475569' }}>Type</label>
          <select id="rel-type" value={type} onChange={(e) => setType(e.target.value)} style={{ padding: 8 }}>
          ...
          <label htmlFor="rel-label" style={{ fontSize: 12, color: '#475569' }}>Label (optional)</label>
          <input id="rel-label" value={label} onChange={(e) => { setLabel(e.target.value); setLabelEdited(true); }} placeholder="leave blank to use type" style={{ padding: 8 }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginTop: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => onConfirm(type, label || undefined)} style={{ padding: '6px 10px', borderRadius: 6, background: '#1f6feb', color: '#fff', border: 'none' }}>{onDelete ? 'Update' : 'Add'}</button>
          </div>
        </div>
      </div>
    </div>
